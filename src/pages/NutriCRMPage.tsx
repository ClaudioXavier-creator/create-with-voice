...
export default function NutriCRMPage() {
  ...
  const destino = "/crm";
  const signupLink = session ? destino : `/auth?product=nutricrm&mode=signup&redirect=%2Fcrm`;
  const loginLink = session ? destino : `/auth?product=nutricrm&mode=login&redirect=%2Fcrm`;
