...
export default function Expedicao() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!t) return expedicoes;
    return expedicoes.filter(e =>
      e.numero_nf?.toLowerCase().includes(t) ||
      e.cliente_nome?.toLowerCase().includes(t) ||
      e.cliente_cnpj?.toLowerCase().includes(t)
    );
  }, [expedicoes, busca]);
  ...
  const handleXmlUpload = async (file: File) => {
    try {
      ...
      setOrigem("xml");
      setXmlContent(text);
      toast.success(`XML importado: ${novosItens.length} item(ns), ${dets.length > 0 ? "verifique os lotes" : ""}`);
    } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    if (errItens) {
      toast.error("NF salva, mas erro nos itens: " + errItens.message);
    }

    // Espelha em rastreabilidade (cria registro de saída por lote)
    ...
    if (rastroPayload.length > 0) {
      await supabase.from("rastreabilidade" as any).insert(rastroPayload);
    }

    toast.success("Expedição registrada e rastreabilidade atualizada!");
    setOpen(false);
    resetForm();
    fetchData();
    setSaving(false);
            ...
            <div className="flex items-center gap-2">
              ...
              <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) resetForm(); }}>
                ...
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle>Registrar Expedição / Faturamento</DialogTitle>
                  </DialogHeader>
                      ...
                      <Card>
                        <CardContent className="pt-6">
                          ...
                          <p className="text-xs text-muted-foreground mt-2">
                            O sistema extrai automaticamente: NF, cliente, transportadora, motorista, placa, produtos e <strong>lotes (rastro/nLote)</strong>.
                          </p>
                          {origem === "xml" && (
                            <Badge variant="secondary" className="mt-3">XML carregado — revise e salve</Badge>
                          )}
                        </CardContent>
                      </Card>
                    </TabsContent>
                    ...
                    <div className="border rounded-md p-3 space-y-3">
                      ...
                      {itens.map((it, idx) => (
                        <div key={idx} className="grid grid-cols-12 gap-2 items-end border-b pb-2">
                          ...
                          <div className="col-span-2"><Label className="text-xs">Lote *</Label><Input value={it.lote_produto} onChange={e => updateItem(idx, "lote_produto", e.target.value)} placeholder="lote PA" /></div>
                  ...
                  <DialogFooter>
                    ...
                    <Button onClick={handleSave} disabled={saving}>
                      {saving && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}Salvar Expedição
                    </Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
              ...
              <TableBody>
                {detalheItens.map((it, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell><Badge variant="outline">{it.lote_produto || "—"}</Badge></TableCell>
