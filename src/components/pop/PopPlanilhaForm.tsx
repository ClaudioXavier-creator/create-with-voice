...
export default function PopPlanilhaForm({ planilhaId, periodicidade, userId, popCodigo, popNome, empresaId }: Props) {
  ...
  function buildPdfHtml(): string {
    ...
    const hCells = areas.map((a) => `<th style="border:1px solid #333;padding:4px 6px;background:#d0d0d0;font-size:9px;text-align:center;white-space:nowrap">${esc(a)}</th>`).join("");
