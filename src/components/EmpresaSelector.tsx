  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button 
          variant="ghost" 
          className="w-full h-auto px-3 py-2.5 sm:px-4 sm:py-3 justify-start gap-2 sm:gap-3 rounded-xl bg-sidebar-accent/30 border border-sidebar-border/20 hover:bg-sidebar-accent/50 hover:border-sidebar-border/50 hover:shadow-lg transition-all group overflow-hidden"
        >
          <div className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-primary/20 text-primary group-hover:scale-110 transition-transform shadow-inner shrink-0">
            <Building2 aria-hidden="true" className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
          </div>
          <div className="min-w-0 flex-1 text-left">
            <p className="text-[9px] sm:text-[10px] uppercase tracking-widest text-sidebar-foreground/30 font-bold leading-none mb-0.5 sm:mb-1">Unidade Ativa</p>
            <p className="text-[11px] sm:text-xs font-semibold text-sidebar-foreground/80 truncate leading-tight">
              {empresaAtiva?.nome || "Selecionar"}
            </p>
          </div>
          <ChevronDown aria-hidden="true" className={cn("w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0 transition-transform opacity-30 group-hover:opacity-60", open && "rotate-180")} />
        </Button>
      </PopoverTrigger>
