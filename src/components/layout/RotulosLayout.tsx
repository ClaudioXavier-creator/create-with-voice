...
import { RotulosSidebar } from "./RotulosSidebar";
import LicenseGate from "@/components/LicenseGate";
import EmpresaSelector from "@/components/EmpresaSelector";
import { Button } from "@/components/ui/button";
import { LogOut } from "lucide-react";

const RotulosLayout = ({ children }: { children: React.ReactNode }) => {
    ...
    <SidebarProvider>
      <div className="min-h-screen flex w-full bg-background overflow-hidden">
        ...
        <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
          <header className="h-16 border-b border-border/50 bg-card/50 backdrop-blur-md flex items-center justify-between px-6 shrink-0 z-10">
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex flex-col items-end">
                ...
                <span className="text-[10px] text-teal-600 uppercase tracking-widest font-bold">Nutri_Agro Labels</span>
              </div>
              <Button
                variant="ghost"
                size="icon"
          ...
          <main className="flex-1 overflow-y-auto overflow-x-hidden relative focus:outline-none custom-scrollbar bg-teal-500/[0.01]">
            <div className="container mx-auto p-4 sm:p-6 lg:p-8 animate-fade-in pb-20 sm:pb-8">
              <LicenseGate product="rotulos">
                {children}
              </LicenseGate>
            </div>
          </main>
...
export default memo(RotulosLayout);
